...
object LomajiSplitter {
    ...
    private const val MOE_UNICODE_REGEX = "(b|p|m|f|v|d|t|n|l|z|j|c|q|s|x|g|k|ng|h)?((([iueoa])+(m|n|ng)?)|(m|n|ng))([bdg])?([ˊˇˋ])?"
    private const val PFS_UNICODE_REGEX = "(p|ph|m|f|t|th|n|l|ch|chh|s|k|kh|ng|h)?(([âàáaa̍êèéee̍îìíii̍ôòóoo̍ṳ̂ṳ̀ṳ́ṳṳ̍ûùúuu̍]+(ńg|ǹg|n̂g|n̄g|n̍g|n̆g|ng|ń|ǹ|n̂|n̄|n̍|n̆|n|ḿ|m̀|m̂|m̄|m̍|m̆|m)?)|(ńg|ǹg|n̂g|n̄g|n̍g|n̆g|ng|ń|ǹ|n̂|n̄|n̍|n̆|n|ḿ|m̀|m̂|m̄|m̍|m̆|m))([ptk])?"
