...
object PfsInputConverter {
    private val TAG = PfsInputConverter::class.java.simpleName

    private val sPfsWordExtractPattern = Pattern.compile("(?:(p|ph|m|f|t|th|n|l|ch|chh|s|k|kh|ng|h)?([aiueoṳ]+(n|ng|m)(?:(ng|m|n|)|([ptk]))?([1-9])?|(p|ph|m|f|t|th|n|l|ch|chh|s|k|kh|ng|h)-?-?))", Pattern.CASE_INSENSITIVE)

    fun convertPfsNumberRawInputToPfsWords(input: String?): String? {

        if (input == null) {
            return null
        }

        val matcher = sPfsWordExtractPattern.matcher(input)
        ...
        while (matcher.find()) {
            ...
            val pfsNumber = convertPfsRawInputTopfsNumber(foundHakkaWord)
            val Pfs = convertpfsNumberToPfs(pfsNumber)
            if (BuildConfig.DEBUG_LOG) {
                Log.d(TAG, "foundHakkaWord=$foundHakkaWord, Pfs=$Pfs")
            }
    ...
    private fun convertPfsRawInputTopfsNumber(foundHakkaWord: String): String {
        var pfsNumber = foundHakkaWord
/*
...
*/
    ...
    private fun convertpfsNumberToPfs(pfsNumber: String): String {
        val fixedpfsNumber = ConverterUtils.fixLomajiNumber(pfsNumber)

        if (fixedpfsNumber.length <= 1) {
            return fixedpfsNumber
        }

        if (BuildConfig.DEBUG_LOG) {
            Log.d(TAG, "fixedpfsNumber=$fixedpfsNumber")
        }

        var number = ""

        val lastCharString = fixedpfsNumber.substring(fixedpfsNumber.length - 1)
        if (BuildConfig.DEBUG_LOG) {
        ...
        val PfsWithoutNumber = fixedpfsNumber.substring(0, fixedpfsNumber.length - 1)
        ...
        if (PfsSianntiauPosition == null) {
            ...
            if (str2.isNullOrEmpty()) {
                println("Pfs.spfsNumberToPfsUnicodeHashMap not found: $pfsNumber, $str2pfsNumber")
